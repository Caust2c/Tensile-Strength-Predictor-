...
param_grid = {
    ...
    'epsilon': [0.01, 0.1, 0.2, 0.5],  # Tolerance for error
...
grid_search = GridSearchCV(
    ...
    n_jobs=-1, 
    verbose=1,
    scoring='r2'  # Use R² for evaluation during the search
)

grid_search.fit(X_train_scaled, y_train)

best_params = grid_search.best_params_
print(f"Best Hyperparameters for SVR: {best_params}")
...
plt.title("Predicted vs Actual Tensile Strength (SVR - Hyperparameter Tuned)", fontsize=14)
...
plt.savefig('tensile_strength_prediction_SVR_tuned.png')
plt.show()
